...
function h2(e7, t5) {
  return __async(this, null, function* () {
    ...
    if (t5.dataModel?.relationshipTypes) for (const i3 of t5.dataModel.relationshipTypes) i3.name && n2.set(i3.name, []);
    for (const i3 of e7) n2.has(i3.typeName) && n2.get(i3.typeName)?.push(i3.id);
    for (const [o4, l8] of n2) {
      if (l8.length < 1) continue;
      const e8 = new s5({
        openCypherQuery: `MATCH (n)-[r:${o4}]->(m) WHERE id(r) in $ids RETURN id(n), labels(n)[0], id(m), labels(m)[0]`,
        bindParameters: {
          ids: l8
        }
      });
      s8.push(T(t5, e8).then((e9) => __async(this, null, function* () {
    ...
    return yield Promise.all(s8), r5;
  });
}
function T4(e7, t5) {
  return __async(this, null, function* () {
    t5 ??= false;
    const r5 = {
      generateAllSublayers: t5,
      namedTypeDefinitions: /* @__PURE__ */ new Map()
    };
    return yield I(e7).then((e8) => {
      D(e8, r5);
    }), r5;
  });
}
function E3(e7) {
  return __async(this, null, function* () {
    const r5 = yield r3(), n2 = new r5.MapOfObjectIdentifierSets();
    M(n2, r5, e7);
...
function A(e7) {
  return __async(this, null, function* () {
    const r5 = new (yield r3()).MapOfObjectIdentifierSetsDecoder(), n2 = r5.decode(new Uint8Array(e7)), s8 = /* @__PURE__ */ new Map();
...
function D(e7, t5) {
  for (const [n2, s8] of e7) {
    const e8 = r(t5.namedTypeDefinitions, n2, () => ({
      useAllData: false,
      members: /* @__PURE__ */ new Map()
    }));
    for (const t6 of s8) e8.members.has(t6) || e8.members.set(t6, {
      id: t6
    });
  }
  return t5;
  ...
  writeToStore(o4, n2, p8) {
    ...
    return o4.forEach((o5) => {
      ...
      if (p8 && o5.properties[p8] && (c5 = ot(o5.properties[p8])), "originId" in o5 && "destinationId" in o5 && (o5.properties[s7] = o5.originId, o5.properties[i2] = o5.destinationId), o5.properties[n2] = o5.id, o5.id && this._featureLookup.has(o5.id) && this._featureLookup.get(o5.id).attributes) {
        ...
        p8 && o5.properties[p8] && (i3[p8] = y2(o5.properties[p8])), u2 = new e4(c5 ? JSON.parse(JSON.stringify(c5)) : e7?.geometry ? JSON.parse(JSON.stringify(e7.geometry)) : null, i3, null, o5.properties[n2]);
...
var M2 = class extends S {
  constructor(e7) {
    super(e7), this._processingCacheUpdatesLookup = /* @__PURE__ */ new Map(), this.knowledgeGraph = null, this.inclusionModeDefinition = {
    ...
    e7.knowledgeGraph.dataModel.entityTypes?.forEach((i3) => {
      i3.name && (t5.set(i3.name, "entity"), this._processingCacheUpdatesLookup.set(i3.name, []), e7.inclusionModeDefinition && !e7.inclusionModeDefinition?.generateAllSublayers || this.entityTypeNames.add(i3.name), i3.properties?.forEach((e8) => {
        e8.geometryType && "esriGeometryNull" !== e8.geometryType && this.geographicLookup.set(i3.name, {
          name: e8.name ?? "",
          geometryType: e8.geometryType
        });
      }));
    }), e7.knowledgeGraph.dataModel.relationshipTypes?.forEach((i3) => {
      i3.name && (t5.set(i3.name, "relationship"), this._processingCacheUpdatesLookup.set(i3.name, []), e7.inclusionModeDefinition && !e7.inclusionModeDefinition?.generateAllSublayers || this.relationshipTypeNames.add(i3.name), i3.properties?.forEach((e8) => {
        e8.geometryType && "esriGeometryNull" !== e8.geometryType && this.geographicLookup.set(i3.name, {
          name: e8.name ?? "",
          geometryType: e8.geometryType
        });
      }));
    }), e7.inclusionModeDefinition?.namedTypeDefinitions.forEach((i3, s8) => {
      if ("entity" === t5.get(s8)) this.entityTypeNames.add(s8);
      else {
        if ("relationship" !== t5.get(s8)) return n.getLogger(this).warn(`A named type, ${s8}, was in the inclusion list that wasn't in the data model and will be removed`), void e7.inclusionModeDefinition?.namedTypeDefinitions.delete(s8);
        this.relationshipTypeNames.add(s8);
      }
      const r5 = /* @__PURE__ */ new Map();
      i3.members?.forEach((e8) => {
    ...
    e7.forEach(({
      ...
      if (this.inclusionModeDefinition.namedTypeDefinitions.has(e8)) {
        if (this.inclusionModeDefinition.namedTypeDefinitions.has(e8)) {
          ...
          i3.members || (i3.members = /* @__PURE__ */ new Map()), i3.members.set(t5, {
            id: t5
          }), r(this.memberIdTypeLookup, t5, () => /* @__PURE__ */ new Set()).add(e8);
        }
      } else {
        const i3 = /* @__PURE__ */ new Map();
        i3.set(t5, {
          id: t5
        }), this.inclusionModeDefinition.namedTypeDefinitions.set(e8, {
          useAllData: false,
          members: i3
        }), r(this.memberIdTypeLookup, t5, () => /* @__PURE__ */ new Set()).add(e8);
      }
  ...
  getData(e7, t5, i3) {
    return __async(this, null, function* () {
      if (t5.objectType.name && this.inclusionModeDefinition?.namedTypeDefinitions && this.inclusionModeDefinition.namedTypeDefinitions.size > 0 && !this.inclusionModeDefinition.namedTypeDefinitions.has(t5.objectType.name)) return [];
      let n2;
      if (n2 = e7 || new b({
        where: "1=1",
        outFields: ["*"]
      }), "link-chart" === t5.parentCompositeLayer.type) {
        const e8 = t5.parentCompositeLayer, i4 = this._processingCacheUpdatesLookup.get(t5.objectType.name ?? ""), o4 = n2.outFields;
        o4 && 1 === o4.length && o4[0] === w4 && "1=1" === n2.where || (yield Promise.all(i4 ?? []));
        const s8 = this.sublayerCaches.has(t5.objectType.name ?? "") ? Array.from(this.sublayerCaches.get(t5.objectType.name)?.values()) : [], r5 = [];
    ...
    return __async(this, null, function* () {
      ...
      if (e7.forEach((e8) => {
        if (this.memberIdTypeLookup.has(e8)) for (const t6 of this.memberIdTypeLookup.get(e8)) {
          ...
          r5.has(t6) ? r5.get(t6)?.push(e8) : r5.set(t6, [e8]);
        }
      }), t5 && 0 !== t5?.length) {
      ...
      return r5.forEach((e8, r6) => {
        let a4;
        a4 = t5 && 0 !== t5?.length ? `MATCH (n:${r6})-[r:${o4}]-(m) WHERE id(n) IN $ids RETURN id(r), type(r), id(m), labels(m)[0]` : `MATCH (n:${r6})-[r]-(m) WHERE id(n) IN $ids RETURN id(r), type(r), id(m), labels(m)[0]`;
        const p8 = new Promise((t6) => {
          (() => __async(this, null, function* () {
            const t7 = (yield T(this.knowledgeGraph, new s5({
              openCypherQuery: a4,
              bindParameters: {
                ids: e8
              }
        ...
        s8.push(p8);
      }), this.refreshCacheContent(), yield Promise.all(s8), i3;
    });
  }
  getAttachedRelationships(e7, t5) {
    return __async(this, null, function* () {
      const i3 = [], o4 = "MATCH (n)-[r]->(m) WHERE id(n) IN $nodeIds AND id(m) in $nodeIds AND NOT id(r) IN $relationshipExclusionIds return id(r), type(r)", s8 = (yield T(this.knowledgeGraph, new s5({
        openCypherQuery: o4,
        bindParameters: {
          nodeIds: e7,
          relationshipExclusionIds: t5
        }
      }))).resultRowsStream.getReader();
      try {
        for (; ; ) {
  ...
  refreshCacheContent(e7, t5, n2, s8 = true) {
    return __async(this, null, function* () {
      ...
      this.knowledgeGraph.dataModel.entityTypes?.forEach((e8) => {
        ...
        if (this.memberIdTypeLookup.has(e8)) for (const t6 of this.memberIdTypeLookup.get(e8)) p8.has(t6) ? p8.get(t6)?.push(e8) : p8.set(t6, [e8]);
      }) : this.inclusionModeDefinition?.namedTypeDefinitions?.forEach((e8, t6) => {
        e8.useAllData ? p8.set(t6, null) : e8.members && e8.members.forEach((e9) => {
          p8.has(t6) && null !== p8.get(t6) ? p8.get(t6)?.push(e9.id) : p8.set(t6, [e9.id]);
        });
      }) : (this.knowledgeGraph.dataModel.entityTypes?.forEach((e8) => {
      ...
      for (const [m5, l8] of p8) {
        const e8 = new Promise((e9) => {
          const a5 = () => __async(this, null, function* () {
            const e10 = /* @__PURE__ */ new Set(), a6 = [];
            let p9, h3 = "", c5 = false;
            if (t5 || d4.get(m5)?.properties?.forEach((t6) => {
              t6.name && e10.add(t6.name);
            }), n2 && this.geographicLookup.has(m5)) {
              const t6 = this.geographicLookup.get(m5)?.name;
              t6 && e10.add(t6);
            }
            ...
            const f5 = (yield T(this.knowledgeGraph, p9)).resultRowsStream.getReader();
            for (; ; ) {
              const {
                done: e11,
                value: t6
              ...
              const n3 = r5.writeToStore(i3, w4, this.geographicLookup.get(m5)?.name);
              this.sublayerCaches.has(m5) || this.sublayerCaches.set(m5, /* @__PURE__ */ new Map()), s8 && !this.inclusionModeDefinition?.namedTypeDefinitions.has(m5) && this.inclusionModeDefinition?.namedTypeDefinitions.set(m5, {
                useAllData: false,
                members: /* @__PURE__ */ new Map()
              }), s8 && !this.inclusionModeDefinition?.namedTypeDefinitions.get(m5).members && (this.inclusionModeDefinition.namedTypeDefinitions.get(m5).members = /* @__PURE__ */ new Map());
              const p10 = this.sublayerCaches.get(m5);
              n3.forEach((e12) => {
                p10?.set(e12.attributes[w4], e12), s8 && !this.inclusionModeDefinition?.namedTypeDefinitions.get(m5).members.has(e12.attributes[w4]) && (this.inclusionModeDefinition?.namedTypeDefinitions.get(m5).members.set(e12.attributes[w4], {
                  id: e12.attributes[w4]
                }), r(this.memberIdTypeLookup, e12.attributes[w4], () => /* @__PURE__ */ new Set()).add(m5));
              });
            }
          });
          a5().then(() => {
        ...
        a4.push(e8), this._processingCacheUpdatesLookup.get(m5)?.push(e8);
      }
  ...
  removeFromLayer(e7) {
    ...
    for (const n2 of e7) t5.add(n2.typeName), 1 === this.memberIdTypeLookup.get(n2.id)?.size ? this.memberIdTypeLookup.delete(n2.id) : this.memberIdTypeLookup.get(n2.id)?.delete(n2.typeName), this.inclusionModeDefinition?.namedTypeDefinitions.forEach((e8, t6) => {
      t6 === n2.typeName && e8.members?.has(n2.id) && e8.members.delete(n2.id);
    });
    t5.forEach((e8) => {
      this.sublayerCaches.get(e8)?.forEach((t6, n2) => {
        i3.has(n2) && this.sublayerCaches.get(e8)?.delete(n2);
      });
    });
  }
  retrieveDataFromService(e7, t5, n2) {
    return __async(this, null, function* () {
      const o4 = o3.getInstance(), r5 = /* @__PURE__ */ new Set(), a4 = [];
      let f5, b3 = "", T6 = [];
      const D3 = "relationship" === t5.graphType, I3 = this.inclusionModeDefinition?.namedTypeDefinitions?.get(t5.objectType.name)?.useAllData, M3 = t5.parentCompositeLayer.sublayerIdsCache.get(t5.objectType.name);
      let E4 = !I3 && M3 ? Array.from(M3).sort() : null;
      if (this.inclusionModeDefinition?.namedTypeDefinitions?.get(t5.objectType.name)?.useAllData) this.inclusionModeDefinition?.namedTypeDefinitions?.get(t5.objectType.name)?.useAllData && null != e7.objectIds && (E4 = e7.objectIds);
        ...
        if (this.inclusionModeDefinition?.namedTypeDefinitions.has(t5.objectType.name) && (!this.inclusionModeDefinition.namedTypeDefinitions.get(t5.objectType.name)?.members || this.inclusionModeDefinition.namedTypeDefinitions.get(t5.objectType.name)?.members?.size < 1)) return e7.objectIds = [], [];
        e7.objectIds = E4;
      }
      ...
      if (null != e7.geometry) {
        ...
        let o5;
        const h3 = t5.parentCompositeLayer.dataManager.knowledgeGraph.serviceDefinition, y5 = h3?.spatialReference, w5 = h3?.serviceCapabilities?.geometryCapabilities;
        let T7 = w5?.geometryMaxBoundingRectangleSizeX, I4 = w5?.geometryMaxBoundingRectangleSizeY;
        if (n3?.extent?.spatialReference && !n3.spatialReference?.isWGS84 ? (yield N(n3.extent.spatialReference, g), o5 = K(n3.extent, g)) : o5 = n3.extent, T7 && I4 && y5) {
          if (4326 !== y5.wkid) {
            const e8 = new w({
              spatialReference: y5,
              xmax: T7,
        ...
        if (null != e7.where && "1=1" !== e7.where) {
          const i3 = yield e3(e7.where.toUpperCase(), t5.fieldsIndex);
        ...
        let i3 = "";
        if (null != e7.where && "1=1" !== e7.where) {
          const n4 = yield e3(e7.where, t5.fieldsIndex);
          t5.fields.forEach((e8) => {
      ...
      const k2 = (yield T(t5.parentCompositeLayer.dataManager.knowledgeGraph, f5, n2)).resultRowsStream.getReader();
      for (; ; ) {
        const {
          done: e8,
          value: i3
        ...
        T6 = T6.concat(o4.writeToStore(n3, w4, t5.parentCompositeLayer.dataManager.geographicLookup.get(t5.objectType.name)?.name));
      }
      return T6;
    });
  }
};
e2([y()], M2.prototype, "knowledgeGraph", void 0), e2([y()], M2.prototype, "inclusionModeDefinition", void 0), e2([y()], M2.prototype, "entityTypeNames", void 0), e2([y()], M2.prototype, "relationshipTypeNames", void 0), e2([y()], M2.prototype, "geographicLookup", void 0), e2([y()], M2.prototype, "sublayerCaches", void 0), e2([y()], M2.prototype, "nodeConnectionsLookup", void 0), e2([y()], M2.prototype, "relationshipConnectionsLookup", void 0), e2([y()], M2.prototype, "memberIdTypeLookup", void 0), M2 = e2([a2("esri.layers.knowledgeGraph.KnowledgeGraphLayerDataManager")], M2);
...
var p7 = (s8) => {
  ...
  return e2([y(t4.fields)], p8.prototype, "fields", void 0), e2([y(t4.fieldsIndex)], p8.prototype, "fieldsIndex", void 0), p8 = e2([a2("esri.layers.knowledgeGraph.KnowledgeGraphSublayerBase")], p8), p8;
};
...
function re(e7) {
  ...
  return function(e8, t6) {
    ...
    return r6.enabled ? __spreadValues(__spreadValues({}, i3), r6) : r6;
  };
}
function oe() {
  return {
...
function ie(e7, t5) {
  ...
  return r5 && (o4 = __spreadValues(__spreadValues({}, o4), ne.call(this, e7, t5))), o4;
}
function ne(e7, t5) {
  return {
    ignoreOrigin: this.originIdOf(t5) > e.DEFAULTS
  };
}
var se = class extends p7(c3(c(l(p3(l5(t(f3(S2(f2))))))))) {
  constructor(e7) {
  ...
  get fields() {
    const e7 = [];
    return this.objectType?.properties?.forEach((t5) => {
      ...
      e7.push(y3.fromJSON({
        ...
        editable: t5.editable,
        nullable: t5.nullable
      }));
    }), e7.push(y3.fromJSON({
  ...
  get geometryType() {
    ...
    const e7 = this.parentCompositeLayer?.dataManager.geographicLookup.get(this.objectType?.name), t5 = e7?.geometryType;
    return t5 && "esriGeometryNull" !== t5 ? o.fromJSON(t5) : null;
  }
  get geometryFieldName() {
    if ("link-chart" === this.parentCompositeLayer?.type) return D2;
  ...
  get hasM() {
    const e7 = this.parentCompositeLayer?.dataManager.geographicLookup.get(this.objectType?.name), t5 = e7?.name, r5 = t5 ? this.objectType?.properties?.[t5] : null;
    return r5?.hasM ?? false;
  }
  get hasZ() {
    const e7 = this.parentCompositeLayer?.dataManager.geographicLookup.get(this.objectType?.name), t5 = e7?.name, r5 = t5 ? this.objectType?.properties?.[t5] : null;
    return r5?.hasZ ?? false;
  }
  set renderer(e7) {
    p5(e7, this.fieldsIndex), this._set("renderer", e7);
  }
  get renderer() {
    return this._isOverridden("renderer") ? this._get("renderer") : "link-chart" === this.parentCompositeLayer?.type ? "relationship" === this.graphType ? t2(o2(o.toJSON("polyline")).renderer) : t2(o2(o.toJSON("point")).renderer) : t2(o2(o.toJSON(this.geometryType)).renderer);
  }
  ...
  queryFeatures(e7, t5) {
    return __async(this, null, function* () {
      const {
        resolvedQuery: r5,
        queryEngine: o4
      } = yield this._setupQueryObjects(e7), i3 = d.fromJSON(yield o4.executeQuery(r5.toJSON(), t5?.signal));
      return i3.features.forEach((e8) => {
        e8.sourceLayer = this;
      }), i3;
    });
  }
  queryFeaturesJSON(e7, t5) {
    return __async(this, null, function* () {
      const {
        resolvedQuery: r5,
        queryEngine: o4
      } = yield this._setupQueryObjects(e7);
      return yield o4.executeQuery(r5.toJSON(), t5?.signal);
    });
  }
  queryFeatureCount(e7, t5) {
    return __async(this, null, function* () {
      const {
        resolvedQuery: r5,
        queryEngine: o4
      } = yield this._setupQueryObjects(e7);
      return o4.executeQueryForCount(r5.toJSON(), t5?.signal);
    });
  }
  queryExtent() {
    return __async(this, arguments, function* (e7 = {}, t5) {
      const r5 = __spreadProps(__spreadValues({}, e7), {
        returnGeometry: true
      }), {
        resolvedQuery: o4,
        queryEngine: i3
      } = yield this._setupQueryObjects(r5), n2 = yield i3.executeQueryForExtent(o4.toJSON(), t5?.signal);
      let s8;
      return s8 = null != n2.extent?.xmin && null != n2.extent?.xmax && null != n2.extent?.ymin && null != n2.extent?.ymax ? new w(n2.extent) : new w(), {
        count: n2.count,
        extent: s8
      };
    });
  }
  queryObjectIds(e7, t5) {
    return __async(this, null, function* () {
      const r5 = b.from(e7);
      let o4;
      if ("link-chart" === this.parentCompositeLayer.type && this._cachedQueryEngine) o4 = this._cachedQueryEngine;
      else {
        const e8 = yield this.parentCompositeLayer.dataManager.getData(r5, this, t5);
        o4 = this.loadQueryEngine(e8);
      }
      return o4.executeQueryForIds(r5.toJSON(), t5?.signal);
    });
  }
  loadQueryEngine(e7) {
    const t5 = new m3({
      geometryType: o.toJSON(this.geometryType),
      hasM: this.hasM,
      hasZ: this.hasZ
    }), r5 = new $({
  ...
  refreshCachedQueryEngine() {
    return __async(this, null, function* () {
      const e7 = yield this.parentCompositeLayer.dataManager.getData(new b({
        where: "1=1",
        outFields: [w4]
      }), this);
  ...
  _setupQueryObjects(e7, t5) {
    return __async(this, null, function* () {
      const r5 = b.from(e7), o4 = r5.geometry;
      let i3;
      if (o4 && !o4.spatialReference?.isWGS84 && (yield N(o4.spatialReference, g), r5.geometry = K(o4 instanceof j || o4 instanceof m ? o4 : o4.extent, g)), "link-chart" === this.parentCompositeLayer.type && this._cachedQueryEngine) i3 = this._cachedQueryEngine;
      else {
        const e8 = yield this.parentCompositeLayer.dataManager.getData(r5, this, t5);
...
e2([y(ee(a(s2)))], se.prototype, "blendMode", void 0), e2([y()], se.prototype, "capabilities", void 0), e2([y({
  json: {
    origins: {
      "web-scene": {
        write: false
...
})], se.prototype, "charts", void 0), e2([y({
  readOnly: true
})], se.prototype, "defaultPopupTemplate", null), e2([y({
  type: String,
  json: {
    origins: {
      service: {
...
})], se.prototype, "definitionExpression", void 0), e2([y()], se.prototype, "displayField", void 0), e2([y(ee(a(a3)))], se.prototype, "effect", void 0), e2([y()], se.prototype, "elevationInfo", void 0), e2([y(ee(a(p2)))], se.prototype, "featureEffect", void 0), e2([y(ee(a(p4)))], se.prototype, "featureReduction", null), e2([y()], se.prototype, "fields", null), e2([y()], se.prototype, "geometryType", null), e2([y()], se.prototype, "geometryFieldName", null), e2([y({
...
})], se.prototype, "graphType", void 0), e2([y({
  type: String,
  nonNullable: true,
  json: {
    write: {
...
})], se.prototype, "graphTypeName", null), e2([y()], se.prototype, "hasM", null), e2([y()], se.prototype, "hasZ", null), e2([y({
  type: String,
  json: {
    origins: {
      service: {
...
})], se.prototype, "id", void 0), e2([y(ee(a(l2)))], se.prototype, "labelsVisible", void 0), e2([y({
  type: [C],
  json: {
...
})], se.prototype, "labelingInfo", void 0), e2([y({
  readOnly: true,
  json: {
    read: false,
    write: {
...
})], se.prototype, "layerType", void 0), e2([y(ee(a(y4)))], se.prototype, "legendEnabled", void 0), e2([y(ee(a(j2)))], se.prototype, "maxScale", void 0), e2([y(ee(a(u)))], se.prototype, "minScale", void 0), e2([y()], se.prototype, "objectIdField", void 0), e2([y()], se.prototype, "objectType", void 0), e2([y(ee(a(w2)))], se.prototype, "opacity", void 0), e2([y(ee(a(c2)))], se.prototype, "orderBy", void 0), e2([y()], se.prototype, "parentCompositeLayer", void 0), e2([y(ee(a(s3)))], se.prototype, "popupEnabled", void 0), e2([y({
  type: k,
  json: {
    name: "popupInfo",
    write: {
...
})], se.prototype, "popupTemplate", void 0), e2([y({
  type: Number,
  json: {
    write: {
      overridePolicy: ne
...
})], se.prototype, "refreshInterval", void 0), e2([y({
  types: m2,
  json: {
    name: "layerDefinition.drawingInfo.renderer",
    write: oe()
  }
})], se.prototype, "renderer", null), e2([y()], se.prototype, "source", void 0), e2([y()], se.prototype, "spatialReference", null), e2([y({
  type: String,
  json: {
    write: {
      isRequired: true,
...
})], se.prototype, "title", void 0), e2([r2("title")], se.prototype, "writeTitle", null), e2([y({
  json: {
    read: false
  }
})], se.prototype, "type", void 0), e2([y(ee(a(f4)))], se.prototype, "useViewTime", void 0), e2([y({
  type: Boolean,
  json: {
    name: "visibility",
    write: oe()
  }
})], se.prototype, "visible", void 0), se = e2([a2("esri.layers.knowledgeGraph.KnowledgeGraphSublayer")], se);
var ae = se;
...
//# sourceMappingURL=chunk-M27RJZHW.js.map
