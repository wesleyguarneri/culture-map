...
var r = class _r {
  ...
  encodeAttributes(t6, n2, o2, s) {
    ...
    for (const f3 of Object.keys(i6)) {
      const _3 = i6[f3], {
        ...
        precisionFactor: l5,
        isLayout: d4
      } = r3[f3], y3 = d4 ? o2.getLayoutProperty(f3) : o2.getPaintProperty(f3), I4 = y3.interpolator?.getInterpolationRange(n2);
      let m = 0;
      for (const o3 of _3) {
        const {
          offset: r4,
          bufferElementsToAdd: i7
        } = o3;
        if (i7 > 0) {
  ...
  _buildAttributesInfo() {
    ...
    for (const o2 of c3) {
      ...
      for (let e4 = 0; e4 < c4; e4++) {
        const {
          ...
          count: o3,
          normalized: a7
        } = _r._encodingInfo[h3], c5 = n2 * o3, u4 = `${e5}-${true === a7}`;
        let f5 = s[u4], l5 = 0;
        if (!f5 || f5.count + o3 > 4) i6++, f5 = {
          dataIndex: i6,
          count: 0,
          offset: 0
        }, 4 !== o3 && (s[u4] = f5), t6.push({
          location: -1,
          name: "a_data_" + i6,
          count: o3,
          type: e5,
          normalized: a7
        }), l5 = Math.ceil(Math.max(c5 / 4, 1));
        else {
        ...
        _4.push({
          dataIndex: f5.dataIndex,
          offset: f5.offset,
          bufferElementsToAdd: l5
        }), f5.offset += c5, f5.count += o3;
      }
    }
    ...
    for (const n2 of _3) this._locations.set(n2.name, f3++);
    const h2 = this._layoutInfo.opacity;
    if (h2) for (const n2 of h2) this._locations.set(n2.name, f3++);
    this._buildShaderInfo(t6, e3), this._propertyEncodingInfo = e3;
  }
  _buildVertexBufferLayout(t6) {
    const e3 = {}, n2 = this.geometryInfo();
    let o2 = n2[0].stride;
    if (0 === t6.length) e3.geometry = n2;
    else {
      const r3 = [];
      let a6 = o2;
      for (const e4 of t6) o2 += i2(e4.type) * e4.count;
      for (const t7 of n2) r3.push(new t(t7.name, t7.count, t7.type, t7.offset, o2, t7.normalized));
      for (const e4 of t6) r3.push(new t(e4.name, e4.count, e4.type, a6, o2, e4.normalized)), a6 += i2(e4.type) * e4.count;
      e3.geometry = r3;
    }
...
`;
    ...
    for (const h2 of u3) {
      ...
      const {
        ...
        precisionFactor: l5,
        isLayout: d4
      } = f3[h2], y3 = l5 && 1 !== l5 ? " * " + 1 / l5 : "", {
        bytesPerElement: I4,
        count: m
      } = _r._encodingInfo[u4], E2 = (t8) => `a_data_${t8.dataIndex}${a3(m, t8.offset, I4)}`;
...
`;
        ...
        case R2.INTERPOLATED_DATA_DRIVEN: {
          ...
          c3.push({
            name: e3,
            type: "float",
            getValue: (t8, e4, n3, o3) => (d4 ? t8.getLayoutProperty(h2) : t8.getPaintProperty(h2)).interpolator.interpolationUniformValue(n3, e4)
          }), s += `uniform float ${e3};
`;
...
r._encodingInfo = {
  ...
  [N2.R16G16B16A16_DASHARRAY]: {
    dataType: C.UNSIGNED_SHORT,
    bytesPerElement: 2,
    count: 4,
    normalized: false
  },
  [N2.R16G16B16A16_PATTERN]: {
    dataType: C.UNSIGNED_SHORT,
    bytesPerElement: 2,
    count: 4,
    normalized: false
...
t3.ATTRIBUTES = ["circle-radius", "circle-color", "circle-opacity", "circle-stroke-width", "circle-stroke-color", "circle-stroke-opacity", "circle-blur"], t3.GEOMETRY_LAYOUT = [new t("a_pos", 2, C.SHORT, 0, 4)], t3.ATTRIBUTES_INFO = {
  "circle-radius": {
    name: "radius",
    type: N2.R8_UNSIGNED
  },
...
l2.ATTRIBUTES = ["fill-color", "fill-opacity", "fill-pattern"], l2.GEOMETRY_LAYOUT = [new t("a_pos", 2, C.SHORT, 0, 4)], l2.ATTRIBUTES_INFO = {
  "fill-color": {
    name: "color",
    type: N2.R8G8B8A8_COLOR
  },
...
T2.ATTRIBUTES_OUTLINE = ["fill-outline-color", "fill-opacity"], T2.ATTRIBUTES_FILL = ["fill-color", "fill-opacity"], T2.GEOMETRY_LAYOUT = [new t("a_pos", 2, C.SHORT, 0, 8), new t("a_offset", 2, C.BYTE, 4, 8), new t("a_xnormal", 2, C.BYTE, 6, 8)], T2.ATTRIBUTES_INFO_OUTLINE = {
  "fill-outline-color": {
    name: "color",
    type: N2.R8G8B8A8_COLOR
  },
...
i3.ATTRIBUTES = ["line-blur", "line-color", "line-gap-width", "line-offset", "line-opacity", "line-width", "line-pattern", "line-dasharray"], i3.GEOMETRY_LAYOUT = [new t("a_pos", 2, C.SHORT, 0, 16), new t("a_extrude_offset", 4, C.BYTE, 4, 16), new t("a_dir_normal", 4, C.BYTE, 8, 16), new t("a_accumulatedDistance", 2, C.UNSIGNED_SHORT, 12, 16)], i3.ATTRIBUTES_INFO = {
  "line-width": {
    name: "width",
    type: N2.R8_UNSIGNED,
    precisionFactor: 2
  ...
  "line-dasharray": {
    name: "tlbr",
    type: N2.R16G16B16A16_DASHARRAY,
    isOptional: true
  }
};
...
i4.ATTRIBUTES = ["icon-color", "icon-opacity", "icon-halo-blur", "icon-halo-color", "icon-halo-width", "icon-size"], i4.ATTRIBUTES_INFO = {
  "icon-color": {
    name: "color",
    type: N2.R8G8B8A8_COLOR
  },
...
function l3(y3) {
  ...
  if ("boolean" == typeof y3) return i5;
  if ("number" == typeof y3) return e2;
...
function c2(n2, r3) {
  ...
  if ("value" === r3.kind) {
    for (const e3 of y2) if (c2(n2, e3)) return true;
  }
...
var P2 = class _P {
  ...
  evaluate(n2, a6) {
    ...
    if ("interpolate-hcl" === this._operator) {
      const s = j(f3), n3 = j(w), a7 = n3.h - s.h, i7 = y({
        h: s.h + p2 * (a7 > 180 || a7 < -180 ? a7 - 360 * Math.round(a7 / 360) : a7),
        c: p(s.c, n3.c, p2),
        l: p(s.l, n3.l, p2)
      });
      return new u(__spreadValues({
        a: p(f3.a, w.a, p2)
      }, i7));
    }
    if ("interpolate-lab" === this._operator) {
      const e3 = A(f3), n3 = A(w), a7 = y({
        l: p(e3.l, n3.l, p2),
        a: p(e3.a, n3.a, p2),
        b: p(e3.b, n3.b, p2)
      });
...
rt.ops = {
  abs: (t6) => Math.abs(t6[0]),
  acos: (t6) => Math.acos(t6[0]),
  asin: (t6) => Math.asin(t6[0]),
  atan: (t6) => Math.atan(t6[0]),
  ceil: (t6) => Math.ceil(t6[0]),
  cos: (t6) => Math.cos(t6[0]),
  e: () => Math.E,
  floor: (t6) => Math.floor(t6[0]),
  ln: (t6) => Math.log(t6[0]),
  ln2: () => Math.LN2,
  log10: (t6) => Math.log(t6[0]) / Math.LN10,
  log2: (t6) => Math.log(t6[0]) / Math.LN2,
  max: (t6) => Math.max(...t6),
  min: (t6) => Math.min(...t6),
  pi: () => Math.PI,
  round: (t6) => Math.round(t6[0]),
  sin: (t6) => Math.sin(t6[0]),
  sqrt: (t6) => Math.sqrt(t6[0]),
  tan: (t6) => Math.tan(t6[0])
};
var st = class _st {
  constructor(t6) {
...
nt.ops = {
  downcase: (t6) => t6.toLowerCase(),
  upcase: (t6) => t6.toUpperCase()
};
var at = class _at {
  constructor(t6) {
...
var ut = class _ut {
  ...
  static toColor(e3) {
    ...
    if (Array.isArray(e3)) return u.fromArray(e3);
    throw new Error(`"${e3}" is not a color`);
  }
};
ut.types = {
...
var h = class _h {
  constructor(t6, e3) {
    ...
    const h2 = e3?.stops && e3.stops.length > 0;
    if (h2) for (const r3 of e3.stops) r3[1] = this._validate(r3[1], t6);
    if (this.isDataDriven = !!e3 && !!e3.property, this.isDataDriven) {
      if (void 0 !== e3.default && (e3.default = this._validate(e3.default, t6)), h2) switch (e3.type) {
        case "identity":
    ...
    for (const n2 of i6) {
      ...
      if (!n2 || n2 !== a6) {
        const i7 = e3[n2], {
          isLayout: a7,
          isOptional: o3
        } = i7, r4 = a7 ? this.getLayoutProperty(n2) : this.getPaintProperty(n2);
...
var U2 = class {
  constructor(t6, i6, e3) {
    let a6;
    this.allowOverlap = t6.getLayoutValue("icon-allow-overlap", i6), this.ignorePlacement = t6.getLayoutValue("icon-ignore-placement", i6), this.keepUpright = t6.getLayoutValue("icon-keep-upright", i6), this.optional = t6.getLayoutValue("icon-optional", i6), this.rotationAlignment = t6.getLayoutValue("icon-rotation-alignment", i6), this.rotationAlignment === l.AUTO && (this.rotationAlignment = e3 ? l.MAP : l.VIEWPORT), a6 = t6.getLayoutProperty("icon-anchor"), a6?.isDataDriven ? this._anchorProp = a6 : this.anchor = t6.getLayoutValue("icon-anchor", i6), a6 = t6.getLayoutProperty("icon-offset"), a6?.isDataDriven ? this._offsetProp = a6 : this.offset = t6.getLayoutValue("icon-offset", i6), a6 = t6.getLayoutProperty("icon-padding"), a6?.isDataDriven ? this._paddingProp = a6 : this.padding = t6.getLayoutValue("icon-padding", i6), a6 = t6.getLayoutProperty("icon-rotate"), a6?.isDataDriven ? this._rotateProp = a6 : this.rotate = t6.getLayoutValue("icon-rotate", i6), a6 = t6.getLayoutProperty("icon-size"), a6?.isDataDriven ? this._sizeProp = a6 : this.size = t6.getLayoutValue("icon-size", i6);
  }
  update(t6, i6) {
...
var O2 = class {
  constructor(t6, i6, e3) {
    let a6;
    this.allowOverlap = t6.getLayoutValue("text-allow-overlap", i6), this.ignorePlacement = t6.getLayoutValue("text-ignore-placement", i6), this.keepUpright = t6.getLayoutValue("text-keep-upright", i6), this.optional = t6.getLayoutValue("text-optional", i6), this.rotationAlignment = t6.getLayoutValue("text-rotation-alignment", i6), this.rotationAlignment === l.AUTO && (this.rotationAlignment = e3 ? l.MAP : l.VIEWPORT), a6 = t6.getLayoutProperty("text-anchor"), a6?.isDataDriven ? this._anchorProp = a6 : this.anchor = t6.getLayoutValue("text-anchor", i6), a6 = t6.getLayoutProperty("text-justify"), a6?.isDataDriven ? this._justifyProp = a6 : this.justify = t6.getLayoutValue("text-justify", i6), a6 = t6.getLayoutProperty("text-letter-spacing"), a6?.isDataDriven ? this._letterSpacingProp = a6 : this.letterSpacing = t6.getLayoutValue("text-letter-spacing", i6), a6 = t6.getLayoutProperty("text-line-height"), a6?.isDataDriven ? this._lineHeightProp = a6 : this.lineHeight = t6.getLayoutValue("text-line-height", i6), a6 = t6.getLayoutProperty("text-max-angle"), a6?.isDataDriven ? this._maxAngleProp = a6 : this.maxAngle = t6.getLayoutValue("text-max-angle", i6), a6 = t6.getLayoutProperty("text-max-width"), a6?.isDataDriven ? this._maxWidthProp = a6 : this.maxWidth = t6.getLayoutValue("text-max-width", i6), a6 = t6.getLayoutProperty("text-offset"), a6?.isDataDriven ? this._offsetProp = a6 : this.offset = t6.getLayoutValue("text-offset", i6), a6 = t6.getLayoutProperty("text-padding"), a6?.isDataDriven ? this._paddingProp = a6 : this.padding = t6.getLayoutValue("text-padding", i6), a6 = t6.getLayoutProperty("text-rotate"), a6?.isDataDriven ? this._rotateProp = a6 : this.rotate = t6.getLayoutValue("text-rotate", i6), a6 = t6.getLayoutProperty("text-size"), a6?.isDataDriven ? this._sizeProp = a6 : this.size = t6.getLayoutValue("text-size", i6), a6 = t6.getLayoutProperty("text-writing-mode"), a6?.isDataDriven ? this._writingModeProp = a6 : this.writingMode = t6.getLayoutValue("text-writing-mode", i6);
  }
  update(t6, i6) {
...
var l4 = class _l {
  ...
  setPaintProperties(e3, t6) {
    ...
    const s = {
      ...
      id: r3.id,
      source: r3.source,
      "source-layer": r3.sourceLayer,
      minzoom: r3.minzoom,
      maxzoom: r3.maxzoom,
      filter: r3.filter,
      layout: r3.layout,
      paint: t6
    }, i6 = _l._recreateLayer(s, r3), a6 = this.layers.indexOf(r3);
  ...
  setStyleLayerVisibility(e3, t6) {
    ...
    const i6 = {
      ...
      layout: s,
      paint: r3.paint
    }, a6 = _l._recreateLayer(i6, r3), y3 = this.layers.indexOf(r3);
  ...
  _identifyRefLayers() {
    ...
    for (const i6 of this.layers) {
      ...
      if (i6.type === a2.FILL) {
        ...
        r4 += "|" + (a6?.visibility ?? ""), r4 += "|" + i6.minzoom, r4 += "|" + i6.maxzoom, r4 += "|" + JSON.stringify(i6.filter), (e3.hasDataDrivenFill || e3.hasDataDrivenOutline) && (r4 += "|" + s), t6.push({
          key: r4,
          layer: i6
        });
        ...
        y3 += "|" + (a6?.visibility ?? ""), y3 += "|" + i6.minzoom, y3 += "|" + i6.maxzoom, y3 += "|" + JSON.stringify(i6.filter), y3 += "|" + (void 0 !== a6 ? a6["line-cap"] : ""), y3 += "|" + (void 0 !== a6 ? a6["line-join"] : ""), (e3.hasDataDrivenLine || l5) && (y3 += "|" + s), r3.push({
          key: y3,
          layer: i6
        });
      }
...
//# sourceMappingURL=chunk-53QPRVJS.js.map
