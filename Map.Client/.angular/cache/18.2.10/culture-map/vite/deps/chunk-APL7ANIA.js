...
function t(r2, t2, n) {
  ...
  if (r2.blockedRelativeUrls && (s2 = s2.concat(r2.blockedRelativeUrls.map((r3) => new s("url:unsupported", `Relative url '${r3}' is not supported`)))), n) {
    const {
      ignoreUnsupported: e,
      supplementalUnsupportedErrors: r3 = [],
      requiredPropertyChecksDisabled: t3
    } = n;
    e && (s2 = s2.filter(({
      name: e2
    }) => !(o.has(e2) || r3.includes(e2)))), t3 && (s2 = s2.filter((e2) => "web-document-write:property-required" !== e2.name));
  }
  if (s2.length > 0) throw new s(t2.errorName, "Failed to save due to unsupported or invalid content. See 'details.errors' for more detailed information", {
    errors: s2
  });
}
...
//# sourceMappingURL=chunk-APL7ANIA.js.map
