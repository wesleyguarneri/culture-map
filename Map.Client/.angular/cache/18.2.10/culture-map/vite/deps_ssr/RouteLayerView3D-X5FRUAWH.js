...
function v(e2) {
  return e2 instanceof c || e2 instanceof y2 || e2 instanceof C || e2 instanceof T || e2 instanceof j || e2 instanceof S || e2 instanceof w;
}
var S2 = class extends l2(y3) {
  constructor() {
  ...
  initialize() {
    this._set("processor", new O({
      owner: this,
      scaleVisibilityEnabled: true,
      frustumVisibilityEnabled: true
    })), this.addResolvingPromise(this.processor.initializePromise), this._updatingHandles.addOnCollectionChange(() => this._routeItems, (e2) => this._routeItemsChanged(e2), P), this.addResolvingPromise(l3(this).then((e2) => this.fullExtentInLocalViewSpatialReference = e2)), this.addHandles(p(() => this.view?.basemapTerrain?.ready, () => () => this.notifyChange("updating"), {
      once: true
    }));
  }
  destroy() {
    this._updatingHandles.removeAll(), this._set("processor", u(this.processor)), this._get("_routeItems")?.destroy();
  }
  get _routeItems() {
    return new l({
      getCollections: () => [this.layer.pointBarriers, this.layer.polygonBarriers, this.layer.polylineBarriers, this.layer.stops, this.layer.directionLines, this.layer.directionPoints, null != this.layer.routeInfo ? new V([this.layer.routeInfo]) : null]
    });
  }
  _routeItemsChanged(e2) {
    if (e2.removed.length) {
      ...
      for (const r of e2.removed) this.removeHandles(r);
    }
    if (e2.added.length) {
      ...
      for (const r of e2.added) this.addHandles([d(() => r.geometry, (e3, t2) => {
        this._updateGraphic(r, "geometry", e3, t2);
      }), d(() => r.symbol, (e3, t2) => {
        this._updateGraphic(r, "symbol", e3, t2);
      })], r);
    }
  }
  get legendEnabled() {
  ...
  getSuspendInfo() {
    ...
    return e2.outsideOfView = this.processor?.frustumVisibilitySuspended ?? false, e2;
  }
  getHit(e2) {
  ...
  get updatePolicy() {
    return this.processor?.graphicsCore.effectiveUpdatePolicy || C2.SYNC;
  }
  isUpdating() {
    return !(!this.processor?.updating && this.view?.basemapTerrain?.ready);
  }
  get performanceInfo() {
    return new t(this.processor?.graphicsCore.usedMemory ?? 0, this.loadedGraphics.length, -1, -1, 0, this.processor?.graphicsCore.performanceInfo ?? null);
  }
  _updateGraphic(e2, r, t2, s) {
    const o = this._byObjectID.get(e2);
    o[r] = t2, C3.graphic = o, C3.property = r, C3.oldValue = s, C3.newValue = t2, this.processor?.graphicsCore.graphicUpdateHandler(C3);
  }
};
e([y()], S2.prototype, "_routeItems", null), e([y()], S2.prototype, "loadedGraphics", void 0), e([y({
  readOnly: true
})], S2.prototype, "legendEnabled", null), e([y({
  readOnly: true
})], S2.prototype, "visibleAtCurrentScale", null), e([y()], S2.prototype, "layer", void 0), e([y({
  readOnly: true
})], S2.prototype, "processor", void 0), e([y({
  type: Boolean
})], S2.prototype, "slicePlaneEnabled", void 0), S2 = e([a("esri.views.3d.layers.RouteLayerView3D")], S2);
var C3 = {
  graphic: null,
  property: null,
  oldValue: null,
...
//# sourceMappingURL=RouteLayerView3D-X5FRUAWH.js.map
