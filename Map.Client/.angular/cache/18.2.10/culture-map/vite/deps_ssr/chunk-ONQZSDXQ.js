...
function E() {
  ...
  function O() {
    if ("." === e3 && /\d/.test(t2)) return v2 = l2, E2;
    if ("/" === e3 && "*" === t2) return v2 = c2, E2;
    if ("/" === e3 && "/" === t2) return v2 = p, E2;
    if ("." === t2 && j2.length) {
      for (; N(j2); ) ;
      return v2 = l2, E2;
    }
...
function x(t2, e3 = "100", a3 = "300 es") {
  ...
  for (const n2 of t2) if ("preprocessor" === n2.type) {
    const t3 = r3.exec(n2.data);
    if (t3) {
      const r4 = t3[1].replaceAll(/\s{2,}/g, " ");
      if (r4 === a3) return r4;
      if (r4 === e3) return n2.data = "#version " + a3, e3;
      throw new Error("unknown glsl version: " + r4);
    }
  }
  return t2.splice(0, 0, {
    type: "preprocessor",
    data: "#version " + a3
  }, {
    type: "whitespace",
...
function D(t2, e3) {
  for (let a3 = e3 - 1; a3 >= 0; a3--) {
    ...
    if ("whitespace" !== e4.type && "block-comment" !== e4.type) {
      if ("keyword" !== e4.type) break;
      if ("attribute" === e4.data || "in" === e4.data) return true;
    }
  }
  return false;
}
function R(t2, e3, a3, r3) {
  r3 = r3 || a3;
  for (const n2 of t2) if ("ident" === n2.type && n2.data === a3) {
...
function T(t2, e3, a3 = "afterVersion") {
  function r3(t3, e4) {
    for (let a4 = e4; a4 < t3.length; a4++) {
      const e5 = t3[a4];
      if ("operator" === e5.type && ";" === e5.data) return a4;
    }
    return null;
  }
  function n2(t3) {
    ...
    for (let i4 = 0; i4 < t3.length; i4++) {
      ...
      if ("preprocessor" === s4.type && (/#(if|ifdef|ifndef)\s+.+/.test(s4.data) ? ++n3 : /#endif\s*.*/.test(s4.data) && --n3), "afterVersion" !== a3 && "afterPrecision" !== a3 || "preprocessor" === s4.type && s4.data.startsWith("#version") && (o5 = Math.max(o5, i4)), "afterPrecision" === a3 && "keyword" === s4.type && "precision" === s4.data) {
        const e5 = r3(t3, i4);
        if (null === e5) throw new Error("precision statement not followed by any semicolons!");
        o5 = Math.max(o5, e5);
      }
      e4 < o5 && 0 === n3 && (e4 = i4);
    }
  ...
  for (const c3 of e3) t2.splice(s3++, 0, c3);
...
function F(a3, r3) {
  if (a3.startsWith("#version 300")) return a3;
  const n2 = C(a3);
  if (null != n2) return n2;
  const o4 = v(a3);
  if ("300 es" === x(o4, "100", "300 es")) return a3;
  let i3 = null, s3 = null;
  const c3 = {}, p2 = {};
  for (let u2 = 0; u2 < o4.length; ++u2) {
    ...
    switch (a4.type) {
      ...
      case "builtin":
        ...
        } else r3 === H.FRAGMENT_SHADER && "gl_FragDepthEXT" === a4.data && (s3 || (s3 = R(o4, c3, "gl_FragDepth")), a4.data = s3);
        break;
      case "ident":
        if (e.includes(a4.data)) {
          if (r3 === H.VERTEX_SHADER && D(o4, u2)) throw new Error("attribute in vertex shader uses a name that is a reserved word in glsl 300 es");
...
function m2(t2, e3, n2) {
  const i3 = t2.get(e3);
  if (!i3) return t2.set(e3, Array.from(n2)), true;
  const o4 = n2.length;
  if (i3.length !== o4) return t2.set(e3, Array.from(n2)), true;
  for (let r3 = 0; r3 < o4; ++r3) {
    const t3 = n2[r3];
    if (i3[r3] !== t3) {
      for (i3[r3] = t3; r3 < o4; ++r3) i3[r3] = n2[r3];
      return true;
    }
  }
  return false;
...
//# sourceMappingURL=chunk-ONQZSDXQ.js.map
