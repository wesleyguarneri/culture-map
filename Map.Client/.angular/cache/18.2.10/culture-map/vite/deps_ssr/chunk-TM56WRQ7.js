...
function o(h2, i2) {
  const {
    ...
    quality: o2,
    rotation: n2,
    disableDecorations: a2
  } = h2 || {}, r2 = b(h2, i2.padding), l2 = p(h2, {
    width: i2.width - r2.left - r2.right,
    height: i2.height - r2.top - r2.bottom
  }), {
    width: g2,
    height: f2
  } = m(h2, l2), d2 = j(e3), u2 = R[d2];
  return {
    format: d2,
    quality: e(null != o2 ? o2 : u2, 0, 100),
    area: l2,
    width: g2,
    height: f2,
    rotation: n2,
    disableDecorations: !!a2,
    ignoreBackground: !!h2?.ignoreBackground,
    ignorePadding: !!h2?.ignorePadding
  };
}
function n(t, h2) {
  const i2 = o(t, h2), e3 = i2.area, n2 = i2.width / e3.width, a2 = b(i2, h2.padding), r2 = a2.left + a2.right, l2 = a2.top + a2.bottom, g2 = h2.width - r2, f2 = h2.height - l2, d2 = Math.floor(g2 * n2 + r2), u2 = Math.floor(f2 * n2 + l2), c2 = t?.layers ?? [], s3 = i2.ignoreBackground, w2 = i2.ignorePadding;
  return {
    framebufferWidth: d2,
    framebufferHeight: u2,
    region: {
      x: Math.floor(e3.x * n2) + a2.left,
      y: Math.floor(e3.y * n2) + a2.top,
      width: i2.width,
      height: i2.height
    },
    format: i2.format,
...
function a(t, h2, i2) {
  const {
    ctx: e3,
    canvas: o2
  } = l(t, i2), n2 = e3.getImageData(0, 0, t.width, t.height), a2 = u(o2, h2);
  return g(o2), {
    dataUrl: a2,
    data: n2
  };
...
function r(t, h2) {
  const {
    ctx: i2,
    canvas: e3
  } = l(t, h2), o2 = i2.getImageData(0, 0, t.width, t.height);
  return g(e3), o2;
}
function l(t, h2) {
  const i2 = f();
  h2.premultipliedAlpha && q(t), i2.width = t.width, i2.height = t.height;
  const e3 = i2.getContext("2d", {
    willReadFrequently: true
  });
  return e3.putImageData(t, 0, 0), h2.flipY && D(e3), {
    ctx: e3,
    canvas: i2
  };
}
...
function w(t, h2, i2, e3 = 0, o2 = 0, n2 = t.width - e3, a2 = t.height - o2, r2 = false) {
  const {
    ...
    height: f2,
    data: d2
  } = h2, u2 = n2 / g2, c2 = a2 / f2, s3 = Math.ceil(u2 / 2), w2 = Math.ceil(c2 / 2), M2 = t.width;
...
function M(t, o2, n2) {
  ...
  const {
    framebufferWidth: a2,
    framebufferHeight: r2,
    pixelRatio: l2,
    region: g2
  } = t, f2 = b(t, n2), d2 = f2.left + f2.right, u2 = f2.top + f2.bottom, c2 = a2 - d2, s3 = r2 - u2, w2 = Math.min(e2, Math.min((h - d2) / c2, (h - u2) / s3));
  return w2 < i ? t : __spreadProps(__spreadValues({}, t), {
    framebufferWidth: Math.round(c2 * w2) + d2,
    framebufferHeight: Math.round(s3 * w2) + u2,
    pixelRatio: l2 * w2,
    resample: {
      region: {
        x: Math.round((g2.x - f2.left) * w2) + f2.left,
        y: Math.round((g2.y - f2.top) * w2) + f2.top,
        width: Math.round(g2.width * w2),
        height: Math.round(g2.height * w2)
      },
      width: a2,
      height: r2
    }
...
function m(t, h2) {
  ...
  if (null != i2 && null != e3) return {
    width: Math.floor(i2),
    height: Math.floor(e3)
  };
  if (null == i2 && null == e3) return h2;
  const o2 = h2.width / h2.height;
  return null == e3 ? {
    width: Math.floor(i2),
    height: Math.floor(i2 / o2)
  } : {
    width: Math.floor(e3 * o2),
    height: Math.floor(e3)
  };
}
function p(t, h2) {
  ...
  if (t?.area) {
    ...
    if (i2.width = h2.width - i2.x, i2.height = h2.height - i2.y, null != e3 && null != o2) i2.width = Math.min(i2.width, e3), i2.height = Math.min(i2.height, o2);
...
function x(t, h2) {
  const i2 = Math.floor(Math.max(t.x, 0)), e3 = Math.floor(Math.max(t.y, 0)), o2 = {
    x: i2,
    y: e3,
    width: Math.floor(Math.min(t.width, h2.width - i2)),
    height: Math.floor(Math.min(t.height, h2.height - e3))
  }, n2 = o2.width / o2.height, a2 = t.width / t.height;
  if (a2 === n2) return o2;
  if (a2 > n2) {
    const t2 = Math.floor(o2.width / a2), h3 = o2.height - t2;
    return {
      x: o2.x,
      y: Math.floor(o2.y + h3 / 2),
      width: o2.width,
      height: t2
    };
  }
...
//# sourceMappingURL=chunk-TM56WRQ7.js.map
